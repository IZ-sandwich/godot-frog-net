...
/// <summary>
/// S7-BV — S7 Multi-body chaos under the <b>BlendedVelocity</b> policy.
/// Same scenario and network conditions as S7, but cubes + balls are
/// spawned with the <c>policy=blended-velocity</c> metadata so the
/// client-side LocalRigidPropPrediction routes them through the per-tick
/// velocity-blend correction (UE5 PredictiveInterpolation pattern) instead
/// of the default Hysteresis policy.
///
/// <para>
/// Direct comparison target for the default S7 (Hysteresis) so the
/// quantitative summary CSV exposes how BlendedVelocity's correction-rate
/// model trades off divergence vs. rollback cost against the rollback-on-
/// contact model under high entity counts.
/// </para>
/// </summary>
public sealed class S7BV_MultiBodyChaos : IScenario
{
    public string Id => "S7BV-MultiBodyChaos";
    ...
    // OnEntitySpawned (~line 124) and sets Policy = BlendedVelocity for any
    // cube/ball whose spawn metadata contains "policy=blended-velocity".
    private const string PolicyMetadata = "policy=blended-velocity";
